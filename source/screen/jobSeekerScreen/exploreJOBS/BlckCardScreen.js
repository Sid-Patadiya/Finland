...
export default function ExploreJobsBlackCardScreen({ route, navigation }) {
  ...
  var latitude = parseFloat(route?.params.latitude);
  var longitude = parseFloat(route?.params.longitude);

  console.log("latttttttttttttttT>>>>>>", latitude, longitude);
  ...
  const handleShare = async () => {
    ...
    let options = {
      ...
      url: `https://jobportal.atwpl.com/jobs/${searchJobsData[0].id || item.id}`,
      ...
      <View style={{ flex: 1 }}>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <JobsCardDetailsTWO
                ...
                short_description={
                  item.vacancy
                    ? item.vacancy.short_description
                ...
                company={
                  item.vacancy
                    ? item.vacancy.company_name
                    : item.company_name} //|| item.vacancy.company_name
                designation={item.vacancy ? item.vacancy.title : item.title}
                details={
                  item.vacancy
                    ? item.vacancy.job_description
                ...
                urgentVacancy={
                  item.vacancy
                    ? item.vacancy.urgent_vacancy
                    : item.urgent_vacancy}

                experienceYear={
                  item.vacancy
                    ? (parseInt(item.vacancy.experience))
                    : parseInt(item.experience[0].experience)
                }
                CompensationAmount={
                  item.vacancy
                    ? item.currency + `${' '}` + item.vacancy.compensation_amount + `${' '}` + item.vacancy.compensation_details
                    : item.currency + `${' '}` + item.compensation_amount + `${' '}` + item.vacancy.compensation_details
                }
                aboutCompany={
                  item.vacancy ? item.vacancy.about_company : item.about_company
                }
                liscense_required={item.liscense_required}
                no_of_vacancy={
                  item.vacancy ? item.vacancy.no_of_vacancy : item.no_of_vacancy
                }
                EmploymentType={
                  item.vacancy ? item.vacancy.emp_details : item.emp_details
                }
                companyWebsite={item.company_website}

                companyAddress={
                  item.vacancy ?
                    item.vacancy.work_location :
                    item.work_location
                }
                from_date={
                  item.vacancy ? item.vacancy.from_date : item.from_date
                }
                to_date={item.vacancy ? item.vacancy.to_date : item.to_date}

                distance={
                  parseInt(item.distance)
                }
                //   location={item.vacancy ? item.vacancy.work_location : item.work_location}
                highest_degree_name={
                  item.vacancy
                    ? item.vacancy.highest_degree_name
                    : item.degree[0].degree
                }
                degree_completion_year={
                  item.vacancy
                    ? item.vacancy.degree_completion_year
                    : item.degree_completion_year
                }
                Skills={item.skill_list || item.skills}
                latitude={
                  item.vacancy ? item.vacancy.latitude : item.work_location_lat
                }
                longitude={
                  item.vacancy
                    ? item.vacancy.longitude
                    : item.work_location_long
                }
        ...
        <View
          ...
          <View style={styles.whiteBackground}>
            {/* {parseInt(searchJobsData[0].percentage_match) == 0 || parseInt(searchJobsData[0].percentage_match) == null ?
              <Text style={styles.percent}>
                10 %
              </Text>
              :
              <Text style={styles.percent}>
                {(parseInt(searchJobsData[0].percentage_match))} %
              </Text>
            } */}
            <Text style={styles.percent}>
              {loginToken == null ? '' : parseInt(searchJobsData[0].percentage_match) == 0 || parseInt(searchJobsData[0].percentage_match) == null ? '10 %' : parseInt(searchJobsData[0].percentage_match) + ' %'}
            </Text>
