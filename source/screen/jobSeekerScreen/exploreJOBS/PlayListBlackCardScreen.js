...
export default function ExploreJobsBlackCardScreen({ route, navigation }) {
  ...
  const ApplyNow = () => {
    ...
    if (loginToken) {
      let bodydata = {
        vacancy_id: searchJobsData[0].vacancy,
      };
  ...
  const handleShare = async () => {
    let options = {
      ...
      url: `https://jobportal.atwpl.com/jobs/${searchJobsData[0].vacancy}`,
          ...
          renderItem={({ item }) => {
            ...
            return (
              <JobsCardDetailsTWO
                ...
                designation={item.vacancy_title}
                details={item.job_description}
                // SeeFullDetails={showFullDetails}
                SeeFullDetails={true}

                urgentVacancy={item.urgent_vacancy}
                experienceYear={item.experience}
                CompensationAmount={'€  ' + item.compensation_amount + '/pm'} 
                ...
                highest_degree_name={item.highest_degree}
                degree_completion_year={item.degree_completion_year}
                Skills={item.skill_set}
        ...
        <View
          ...
          <View style={styles.whiteBackground}>
            {/* <Text style={styles.percent}>
              {searchJobsData[0].percentage_match
                ? searchJobsData[0].percentage_match
                : 0}
              %
            </Text> */}
