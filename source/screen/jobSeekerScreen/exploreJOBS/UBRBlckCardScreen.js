...
export default function ExploreJobsBlackCardScreen({ route, navigation }) {
  ...
  // console.log("fafdfdfdfgdgdfgd", currentLatitude, currentLongitude);
  ...
  const handleShare = async () => {
    let options = {
      ...
      url: `https://jobportal.atwpl.com/jobs/${searchJobsData[0].vacancy.vacancy_id}`,
      ...
      <View style={{ flex: 1 }}>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <JobsCardDetailsTWO
                ...
                company={item.vacancy.company_name}
                designation={item.vacancy.title}
                details={item.vacancy.job_description}
                SeeFullDetails={true}
                urgentVacancy={item.vacancy.urgent_vacancy}
                experienceYear={item.vacancy.experience}
                short_description={item.vacancy.short_description}
                CompensationAmount={item.vacancy.currency + `${' '}`+ item.vacancy.compensation_amount + `${' '}` + item.vacancy.compensation_details}
                aboutCompany={item.vacancy.about_company}
                liscense_required={item.vacancy.liscense_required}
                no_of_vacancy={item.vacancy.no_of_vacancy}
                EmploymentType={item.vacancy.emp_details}
                companyWebsite={item.company_website}
                companyAddress={item.vacancy.work_location}
                from_date={item.vacancy.from_date}
                to_date={item.vacancy.to_date}
                distance={item.vacancy.search_radius}
                highest_degree_name={item.vacancy.highest_degree_name}
                degree_completion_year={item.vacancy.degree_completion_year}
                Skills={item.skill_list}
                latitude={parseFloat(item.vacancy.latitude)}
                longitude={parseFloat(item.vacancy.longitude)}
        ...
        <View
          ...
          <View style={styles.whiteBackground}>
            {parseInt(searchJobsData[0].percentage_match) == null || parseInt(searchJobsData[0].percentage_match) == 0 ?
              <Text style={styles.percent}>10 % </Text> :
              <Text style={styles.percent}> {parseInt(searchJobsData[0].percentage_match)} % </Text>
            }
